function validatePasswords(input) {
    ...
    for (const line of lines) {
        ...
        const [, minStr, maxStr, targetChar, password] = match;
        const min = parseInt(minStr);
        const max = parseInt(maxStr);
        const charCount = (password.match(new RegExp(targetChar, 'g')) || []).length;
...
function validatePasswordsTwo(input) {
    const lines = input.trim().split('\n');
    let validCount = 0;
    ...
    for (const line of lines) {
        ...
        const [, pos1Str, pos2Str, targetChar, password] = match;
        const pos1 = parseInt(pos1Str) - 1;
        const pos2 = parseInt(pos2Str) - 1;
        const char1 = pos1 < password.length ? password[pos1] : '';
        const char2 = pos2 < password.length ? password[pos2] : '';

        const pos1Match = char1 === targetChar;
        const pos2Match = char2 === targetChar;
        const isValid = pos1Match !== pos2Match;
...
const result = validatePasswords(actualInput);
console.log("=== 예제 테스트 ===");
console.log(`\n총 유효한 패스워드 개수: ${result}`);

console.log("\n=== Part 2: 위치 기반 정책 ===");
const result2 = validatePasswordsTwo(actualInput);
console.log(`총 유효한 패스워드 개수: ${result2}`);
